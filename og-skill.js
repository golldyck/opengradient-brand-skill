...
(function (global) {
  ...
  function initScrollReveal() {
    if (!('IntersectionObserver' in window)) {
      document.querySelectorAll('.og-reveal, .og-reveal--scale').forEach(function (el) {
        el.classList.add('og-revealed', 'og-visible');
      });
      return;
    }
    /* Standard og-reveal (legacy .og-visible + new .og-revealed) */
    var revealObs = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          entry.target.classList.add('og-visible', 'og-revealed');
  ...
  /* ============================================================
     NAV SCROLL BLUR (real opengradient.ai behaviour)
     blur(0) → blur(24px) + subtle shadow on scroll
  ============================================================ */
  function initNavScrollBlur() {
    var nav = document.querySelector('.og-nav');
    if (!nav) return;
    var scrolled = false;
    function onScroll() {
      ...
      if (shouldBlur !== scrolled) {
        scrolled = shouldBlur;
        nav.classList.toggle('og-nav--scrolled', scrolled);
      }
    }
  ...
  var OGBrand = {
    ...
    initNavScrollBlur: initNavScrollBlur,
