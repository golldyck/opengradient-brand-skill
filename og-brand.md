...
13. **LOGO — brand kit only, ONE element, nothing next to it:**
    ...
    **Correct footer logo:**
    ```html
    <div data-og-logo="wordmark" style="height:22px;opacity:.5;"></div>
    ```
...
<extracted_site_patterns>
## Verified Design Patterns — Extracted from live opengradient.ai (March 2026)

### Light Hero Gradient (EXACT — from computed CSS on homepage)
The actual opengradient.ai homepage uses THIS gradient (not dark navy):
```css
/* Hero section background — light teal gradient */
background: linear-gradient(to bottom, #97deed 0%, #7adeed 30%, #e9f8fc 60%, #ffffff 100%);
```
Use `og-light` theme when building pages that should match the real OG website.

### Dark Hero Gradient (used on /opengradient-network and dark product pages)
```css
background: linear-gradient(135deg, #0a0f19 0%, #141e32 50%, #0e4b5b 100%);
```

### Real Homepage Sections (in order):
1. Nav (fixed, glassmorphism, white/14 blur)
2. 2. Hero: "The Network for Open Intelligence" + globe PNG + "Testnet Is Live" badge
   3. 3. NVIDIA Inception banner (thin, scroll-horizontal)
      4. 4. Stats: 100% EVM Compatible | 1500 Models | 2M+ Inferences | 500K+ zkML Proofs
         5. 5. Products grid: Network / Model Hub / SDK / MemSync / Digital Twins / BitQuant
            6. 6. Research Highlights (4 cards with category badge)
               7. 7. Backers: a16z, Coinbase, NVIDIA, Polygon, Celestia, NEAR + testimonials
                  8. 8. What's New (3 blog cards)
                     9. 9. Footer: newsletter + community + jobs + nav
                       
                        10. ### Fluid Container (actual CSS from site — NOT standard Tailwind container):
                        11. ```css
                            .fluid-container { width:100%; margin:0 auto; padding:0 16px; }
                            ...
                            ### Confirmed Keyframe Animations from site:
                            ```css
                            @keyframes scroll { 0% { transform:translate(0); } 100% { transform:translate(-100%); } }
                            @keyframes blink  { 0%,100% { opacity:1; } 50% { opacity:0; } }
                            @keyframes spin   { 100% { transform:rotate(360deg); } }
                            @keyframes pulse  { 50% { opacity:0.5; } }
                            @keyframes bounce { 0%,100% { animation-timing-function:cubic-bezier(.8,0,1,1); transform:translateY(-25%); } 50% { animation-timing-function:cubic-bezier(0,0,.2,1); transform:none; } }
                            ```

                            ### "Testnet Is Live" Badge Pattern:
                            ```html
                            <div style="display:inline-flex;align-items:center;gap:8px;background:white;border-radius:9999px;padding:8px 16px;font-family:'Geist Mono',monospace;font-size:11px;text-transform:uppercase;letter-spacing:0.1em;color:#0e4b5b;">
                              <span style="width:8px;height:8px;border-radius:50%;background:#00de0f;animation:pulse 2s infinite;"></span>
                              TESTNET IS LIVE
                            </div>
                            ```

                            ### Button Style (exact from site — font-mono uppercase pill):
                            ```html
                            <!-- Dark primary (on light backgrounds) -->
                            <a href="#" style="display:inline-flex;align-items:center;background:#0a0f19;color:white;font-family:'Geist Mono',monospace;font-size:11px;text-transform:uppercase;letter-spacing:0.1em;padding:12px 24px;border-radius:9999px;text-decoration:none;">ENTER PORTAL</a>

                            <!-- White outline (secondary on light) -->
                            <a href="#" style="display:inline-flex;align-items:center;border:1px solid #0a0f19;color:#0a0f19;font-family:'Geist Mono',monospace;font-size:11px;text-transform:uppercase;letter-spacing:0.1em;padding:12px 24px;border-radius:9999px;text-decoration:none;">EXPLORE MODELS</a>
                            ```

                            ### Product Icon Style (32×32, stroke-width 1.2, currentColor):
                            All product icons use: fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"
                            See design-extracted/svg-icons-extracted.md for full SVG paths.

                            ### Confirmed Color Tokens (from computed CSS):
                            ```
                            Primary:  #24bce3 (main blue — CTAs, icons, links)
                            Soft:     #50c9e9 (soft accent)
                            Light:    #bdebf7 (body text on dark)
                            Pale:     #e9f8fc (light surface)
                            Teal:     #0e4b5b (dark text on light, dark surface)
                            Navy:     #141e32 (card bg)
                            Deep:     #0a0f19 (main dark bg)
                            Live:     #00de0f (green status dot)
                            White:    #ffffff
                            ```
                            </extracted_site_patterns>
