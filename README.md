...
```bash
curl -s https://raw.githubusercontent.com/golldyck/opengradient-brand-skill/main/SKILL.md \
  -o ~/.claude/commands/og-brand.md
```

That's it. The `/og-brand` command is now available in every Claude Code session.
...
`SKILL.md` is a [Claude Code custom command](https://docs.anthropic.com/en/docs/claude-code/slash-commands) — a Markdown file that tells Claude exactly how to generate pages: which CDN to use, which brand classes to apply, how to name files, and how to fill in real content from your description.

You can inspect or customize it: [`SKILL.md`](SKILL.md)
...
| File | Description |
|---|---|
| `SKILL.md` | Claude Code skill — install to `~/.claude/commands/og-brand.md` and use `/og-brand` |
